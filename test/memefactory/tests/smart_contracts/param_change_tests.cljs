   ...
   (async/go
     (let [[creator-addr challenger-addr] (web3-eth/accounts @web3)
           ...
           tx-hash (<? (param-change-factory/approve-and-create-param-change
                        (look {:db (contract-address :meme-registry-db)
                          :key :deposit
                          :value (web3/to-wei 1 :ether)
                          :amount deposit})
                        (look {:from challenger-addr})))
           registry-entry (look (->> (registry/param-change-constructed-event-in-tx [:param-change-registry :param-change-registry-fwd] (look tx-hash))
                                     look
                                     :args
                                     :registry-entry))
